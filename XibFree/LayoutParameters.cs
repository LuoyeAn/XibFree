...
namespace XibFree
{
	...
	public class LayoutParameters
	{
        ...
        internal bool ParentIsScorllView(View view)
        {
            var host = view.GetHost();
            if (host == null)
                return false;
		...
		internal nfloat TryResolveWidth(View view, nfloat parentWidth, nfloat parentHeight)
		{
            ...
            switch (WidthUnits)
            {
                case Units.HostRatio:
                    return GetHostSize(view).Width * WidthRatio;
                case Units.ScreenRatio:
                    return GetScreenSize().Width * WidthRatio;
                case Units.Absolute:
                    return Width;
                case Units.ParentRatio:
                    return parentWidth == nfloat.MaxValue ? nfloat.MaxValue : parentWidth * WidthRatio;
                case Units.AspectRatio:
                    return TryResolveHeight(view, parentWidth, parentHeight) * WidthRatio;
                default:
                    return nfloat.MaxValue;
            }
        }
		...
		internal nfloat TryResolveHeight(View view,nfloat parentWidth, nfloat parentHeight)
		{
            ...
            switch (HeightUnits)
            {
                ...
                case Units.Absolute:
                    return Height;
                case Units.ParentRatio:
                    return parentHeight == nfloat.MaxValue ? nfloat.MaxValue : parentHeight * HeightRatio;
                case Units.AspectRatio:
                    return TryResolveWidth(view, parentWidth, parentHeight) == nfloat.MaxValue ? nfloat.MaxValue : TryResolveWidth(view, parentWidth, parentHeight) * HeightRatio;
                default:
                    return nfloat.MaxValue;
            }
        }
